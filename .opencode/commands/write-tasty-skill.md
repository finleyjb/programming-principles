...
Follow each phase in order. The human partner judges all quality outcomes — never score compliance on their behalf.
...
3. Dispatch each scenario to a subagent WITHOUT the skill, using the code-writer agent (`$ARGUMENTS` determines the skill; scenarios target its subject matter).
